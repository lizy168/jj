...
pub(crate) async fn cmd_absorb(
    ...
    let is_interactive = diff_selector.is_interactive();
    ...
    let mut commits_to_check = selected_trees.target_commits.keys().collect_vec();
    if is_interactive {
        commits_to_check.push(source.commit().id());
    }
    workspace_command.check_rewritable(commits_to_check).await?;
